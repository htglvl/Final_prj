...
# |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def reset(state_dim, act_dim):
    print("Resetting states, actions, and rewards...")
    states = torch.zeros((1, state_dim), device="cpu", dtype=torch.float32)
    ...
    return states, actions, rewards, target_return, timesteps

reset_interval = 50 # reset each 50 frames
frame_counter = 0
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
model_name_print = "d:\\trained_models_fine_tune\\rained_models_fine_tune\\model_finetunes_after_dataset_60_in_loops_0"
...
for t in range(max_ep_len):
    ...
    if n_loops % reset_interval == 0:
        states, actions, rewards, target_return, timesteps = reset(state_dim, act_dim)
        print("Resetting states, actions, and rewards...")
        continue
    action = get_action(
        model,
